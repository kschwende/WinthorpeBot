...
# Importing config loads WinthorpeBot's own .env (repo root) and exposes STATE_DIR.
from winthorpe.config import STATE_DIR
from winthorpe.data.json_store import atomic_write_json

ET = ZoneInfo("America/New_York")
STATE_DIR.mkdir(parents=True, exist_ok=True)
GEX_FILE = STATE_DIR / "gex_levels.json"
...
async def compute_gex(n_strikes=100, expiry_date=None, product="SPX",
    ...
    # POSITIVE→NEGATIVE then NEGATIVE→POSITIVE alerts to the user.
    ...
    if live_spot is None:
        from winthorpe.data.spot_source import fetch_spot_via_yfinance
    ...
    if "--json" in args:
        ...
        while True:
            ...
            if product == "SPX":
                ...
                es_file = STATE_DIR / "gex_levels_es.json"
