...
atomic — ``write()`` on Linux isn't atomic for arbitrarily-sized buffers,
so a process killed mid-write (SIGKILL, OOM, power loss, a pre-commit
hook interrupting a running script) leaves a truncated/empty file. Next
reader hits ``ValueError: Expecting value`` and silently falls back to
``{}`` / ``[]`` via the usual ``except Exception: return default``
pattern. You've already seen this: ``options_executions.json.bak`` and
``position_monitor.json.bak`` next to the originals in the premium state
directory are the recovery artifacts.
